...
server <- function(input, output, session) {
  ...
  output$infectedComparisonPlot <- renderPlot({
    ...
    # Select every tenth point for plotting
    thin_time_series_df <- time_series_df[seq(1, nrow(time_series_df), by = 10), ]
    
    par(cex.lab = 1.5, font.lab = 1)
    plot(NA, xlim = range(time_steps), ylim = range(c(thin_time_series_df$I, observed_data$total_infected)), xlab = "Час", ylab = "Інфіковані")
    points(thin_time_series_df$time, thin_time_series_df$I, pch = 19, col = "red", cex = 1)  # Model as red points
    lines(observed_data$time, observed_data$total_infected, col = "black", lwd = 3)  # Observed data as thicker black line
    legend("topleft", legend = c("Прогноз моделі", "Дані спостережень"), pch = c(19, NA), lty = c(NA, 1), col = c("red", "black"), lwd = c(1, 3))
  ...
  output$vaccinatedComparisonPlot <- renderPlot({
    ...
    thin_vaccinated_df <- time_series_df[seq(1, nrow(time_series_df), by = 10), ]
    
    plot(NA, xlim = range(time_steps), ylim = range(c(thin_vaccinated_df$V, observed_data$fully_vaccinated)), xlab = "Час", ylab = "Вакциновані")
    points(thin_vaccinated_df$time, thin_vaccinated_df$V, pch = 19, col = "red", cex = 1)  # Model as red points
    lines(observed_data$time, observed_data$fully_vaccinated, col = "black", lwd = 3)  # Thicker black line for observed data
    legend("topleft", legend = c("Прогноз моделі", "Дані спостережень"), pch = c(19, NA), lty = c(NA, 1), col = c("red", "black"), lwd = c(1, 2))
  ...
  output$recoveredComparisonPlot <- renderPlot({
    ...
    thin_recovered_df <- time_series_df[seq(1, nrow(time_series_df), by = 10), ]
    
    plot(NA, xlim = range(time_steps), ylim = range(c(thin_recovered_df$R, observed_data$total_recovered)), xlab = "Час", ylab = "Одужавші")
    points(thin_recovered_df$time, thin_recovered_df$R, pch = 19, col = "red", cex = 1)  # Model as red points
    lines(observed_data$time, observed_data$total_recovered, col = "black", lwd = 3)  # Thicker black line for observed data
    legend("topleft", legend = c("Прогноз моделі", "Дані спостережень"), pch = c(19, NA), lty = c(NA, 1), col = c("red", "black"), lwd = c(1, 2))
  ...
  output$deathComparisonPlot <- renderPlot({
    ...
    thin_death_df <- time_series_df[seq(1, nrow(time_series_df), by = 10), ]
    
    plot(NA, xlim = range(time_steps), ylim = range(c(thin_death_df$D, observed_data$total_death)), xlab = "Час", ylab = "Померлі")
    points(thin_death_df$time, thin_death_df$D, pch = 19, col = "red", cex = 1)  # Model as red points
    lines(observed_data$time, observed_data$total_death, col = "black", lwd = 3)  # Thicker black line for observed data
    legend("topleft", legend = c("Прогноз моделі", "Дані спостережень"), pch = c(19, NA), lty = c(NA, 1), col = c("red", "black"), lwd = c(1, 2))
